...
int add_client(char *, struct sockaddr, int);
    ...
    while (1) {
        ...
        if (strcmp(command, "add") == 0) {
            ...
            if (index == -1)
                ...
                sendto(clients[first]->fd, "add:O", MAX_MESSAGE_LENGTH, 0, &clients[first]->addr,
                       sizeof(struct addrinfo));
                sendto(clients[second]->fd, "add:X", MAX_MESSAGE_LENGTH, 0, &clients[second]->addr,
                       sizeof(struct addrinfo));
            }
        }
        if (strcmp(command, "move") == 0) {
            ...
            sendto(clients[get_opponent(player)]->fd, buffer, MAX_MESSAGE_LENGTH, 0,
                   &clients[get_opponent(player)]->addr, sizeof(struct addrinfo));
        }
...
void remove_client(char *name) {
    ...
    clients[index] = NULL;
    clients_count--;
    int rival = get_opponent(index);
    if (clients[rival] != NULL) {
        printf("Removing rival: %s\n", clients[rival]->name);
        sendto(clients[rival]->fd, "quit: ", MAX_MESSAGE_LENGTH, 0, &clients[rival]->addr, sizeof(struct addrinfo));
